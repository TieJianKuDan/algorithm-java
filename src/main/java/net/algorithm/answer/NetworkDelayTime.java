...
//        int[][] times = {{1,2,1},{2,1,3}};
        ...
        System.out.println(networkDelayTime(times, 15, 8));
    }
    ...
    public static int networkDelayTime(int[][] times, int n, int k) {
        ...
        for (int[] side : map) {
            Arrays.fill(side, Integer.MAX_VALUE / 2);
        }
        for (int[] time : times) {
        ...
        int res = dijkstra(map, k);
        return res == Integer.MAX_VALUE / 2 ? -1 : res;
    }

    public static int dijkstra(int[][] map, int k) {
        int[] distance = new int[map.length];
        Arrays.fill(distance, Integer.MAX_VALUE / 2);
        boolean[] record = new boolean[map.length];
        int now = k;
        distance[k] = 0;
        while (true) {
            for (int i = 1; i < map[now].length; i++) {
                distance[i] = Math.min(distance[i], distance[now] + map[now][i]);
            }
            record[now] = true;
            now = 0;
            int min = Integer.MAX_VALUE;
            for (int i = 1; i < map[k].length; i++) {
                if (!record[i] && map[k][i] < min) {
                    now = i;
                    min = map[k][i];
                }
            }
